...
async def websocket_interview_endpoint(websocket: WebSocket, client_id: str):
    ...
    try:
        # Let the client know we're initializing Gemini
        try:
            await websocket.send_json({
                "type": "bot_text",
                "content": "Initializing Gemini AI model. This may take a few seconds..."
            })
        except Exception as notify_err:
            logger.error(f"Failed to send initialization notification: {notify_err}")
        ...
        # Connect to Gemini with timeout handling
        try:
            logger.info(f"WS ({client_id}): Connecting to Gemini Live API...")
            async with client.aio.live.connect(model=MODEL, config=config) as live_session:
                logger.info(f"WS ({client_id}): Gemini Live API session active.")
                
                # Send a notification once Gemini is ready
                try:
                    await websocket.send_json({
                        "type": "bot_text",
                        "content": "Gemini is ready. You can now start the interview."
                    })
                except Exception as ready_err:
                    logger.error(f"Failed to send ready notification: {ready_err}")
                    # Continue anyway if we can't send the notification
                
                # --- Main Interview Loop ---
                while True:
                    # Receive user message from WebSocket with timeout
                    logger.info(f"WS ({client_id}): Waiting for message from client...")
                    try:
                        # Add a timeout for receiving messages
                        user_message = await asyncio.wait_for(websocket.receive_text(), timeout=60.0)  # 60 second timeout
                        logger.info(f"WS ({client_id}): Received: '{user_message}'")
                    except asyncio.TimeoutError:
                        logger.warning(f"WS ({client_id}): Receive timeout after 60 seconds")
                        # Send a ping to check if client is still there
                        try:
                            await websocket.send_json({"type": "ping", "content": "Connection check"})
                            logger.info(f"WS ({client_id}): Ping sent to check connection")
                            continue  # Continue the loop after ping
                        except Exception as ping_err:
                            logger.error(f"WS ({client_id}): Ping failed, client likely disconnected: {ping_err}")
                            break  # Exit the loop if ping fails
                    except WebSocketDisconnect:
                        logger.info(f"WS ({client_id}): Client disconnected while waiting for message")
                        break
                    except Exception as e:
                        logger.error(f"WS ({client_id}): Error receiving message: {e}")
                        break
            ...
            logger.error(f"Error with Gemini Live API session: {gemini_err}")
            await websocket.send_json({
                "type": "error", 
                "content": f"Gemini API error: {str(gemini_err)}"
            })
